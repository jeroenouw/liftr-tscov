...
import { getRootNames, getTsConfig, getTsConfigFilePath } from './ts-config'
import { JsonConfig } from './types/json-config.type'
import { ignoreTypes } from './ignore-types'

// tslint:disable-next-line:no-big-function
export async function startLinter(project: string, detail: boolean, debug: boolean, files?: any, oldProgram?: ts.Program): Promise<any> {
  const { configFilePath, dirname } = getTsConfigFilePath(project)
  const config: JsonConfig = getTsConfig(configFilePath, dirname)
  ...
  const rootNames: string[] = await getRootNames(config, dirname)
  ...
  function handleSingleNode(node: ts.Node | undefined, file: string, sourceFile: ts.SourceFile): void {
    ...
    if (ignoreTypes.has(node.kind)) return
